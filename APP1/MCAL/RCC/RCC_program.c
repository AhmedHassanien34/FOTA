...
uint8_t RCC_u8EnableDisablPeripheralClk	(uint8_t Copy_u8PeripheralId ,uint8_t Copy_u8State)
{
	uint8_t Local_u8ErrorState = OK;
	if(Copy_u8PeripheralId < 32) /* from 0 to 31 AHB BUS */
	{
		...
		case RCC_u8_Disable:
			...
			break;
		default:
			Local_u8ErrorState = NOK; /* user entered wrong state */
		}
	}
	...
	{
		Local_u8ErrorState = NOK; /* user entered wrong peripheral number */
	}
	return Local_u8ErrorState;
}
